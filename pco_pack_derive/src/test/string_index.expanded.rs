...
const _: () = {
    ...
    /// Intermediate compressed form for [StringIndex] with metadata fields
    ...
    impl Filter {
        ...
        pub fn new(device_id: impl Into<pco_pack::StringFilter>) -> Self {
            Self {
    ...
    impl pco_pack::PcoSerde for StringIndex {
        type Writer = Writer;
        type Reader = Reader;
        fn write(
            data: Vec<Self>,
            ...
            let mut groups: pco_pack::ahash::HashMap<(String,), Vec<usize>> = Default::default();
    ...
    impl pco_pack::PcoFilter for StringIndex {
        fn filter_bulk(
            reader: &mut Self::Reader,
            field: usize,
            ...
            if field == 0usize {
                match filter {
                    pco_pack::Filter::String(v) => {
                        (reader.device_id != *v).then(|| matches.fill(false));
                    }
                    pco_pack::Filter::InclusionString(values) => {
                        (!values.iter().any(|v| v.as_str() == reader.device_id.as_str()))
                ...
                0usize => {
                    ...
                    match filter {
                        ...
                        pco_pack::Filter::InclusionString(values) => {
                            (!values
                                .iter()
                                .any(|v| v.as_str() == reader.device_id.as_str()))
                                .then(|| matches.fill(false));
                        }
                        _ => {
                            unreachable!(
    ...
    impl pco_pack::PcoPack for StringIndex {
        type Reader = Reader;
        type Chunk = Chunk;
        type Filter = Filter;
        fn write(data: Vec<Self>) -> pco_pack::anyhow::Result<Vec<Self::Chunk>> {
            ...
            for (key, indices) in groups {
                for group_indices in indices.chunks(StringIndex::CHUNK_SIZE) {
                    let mut col_temperature: Vec<i64> = Vec::new();
        ...
        fn resolve_fields(
            ...
            let known: &[&str] = &["device_id", "temperature"];
    ...
    fn row_count(g: &mut Reader) -> pco_pack::anyhow::Result<usize> {
        g.temperature.row_count()
    }
    #[inline]
    fn expand_row(g: &mut Reader, row: usize) -> pco_pack::anyhow::Result<StringIndex> {
        Ok(StringIndex {
    ...
    impl Chunk {
        fn compute_row_count(
            ...
            let count = count;
            let has_loaded_payload = fields.contains(&"temperature") || false;
        ...
        pub fn expand(self) -> pco_pack::anyhow::Result<Vec<StringIndex>> {
            ...
            let loaded_fields: &[&str] = &["temperature"];
        ...
        pub fn filter(
            ...
            query: &pco_pack::serde_json::Value,
            fields: &[&str],
        ) -> pco_pack::anyhow::Result<Vec<StringIndex>> {
            let fields = <StringIndex as pco_pack::PcoPack>::resolve_fields(
                query,
                fields,
            )?;
            let execution_plan = <StringIndex as pco_pack::PcoFilter>::resolve_query(
                query,
            )?;
            let mut reader = Reader {
                device_id: self.device_id.clone(),
            ...
            let sizes = [0, self.temperature.len()];
            plan.sort_by_key(|&i| sizes[execution_plan[i].path[0]]);
            for i in plan {
                <StringIndex as pco_pack::PcoFilter>::filter_step(
                    &mut reader,
...
const _: () = {
    ...
    use pco_pack::anyhow::Context;
    use pco_pack::serde::Serialize;
    /// Intermediate compressed form for [SmolStrIndex] with metadata fields
    /// (index, timestamp bounds) uncompressed and payload columns
    /// stored as compressed ByteBuf. One instance per logical group.
    #[derive(pco_pack::serde::Serialize, pco_pack::serde::Deserialize, Default, Clone)]
    pub struct Chunk {
    ...
    impl pco_pack::PcoSerde for SmolStrIndex {
        type Writer = Writer;
        type Reader = Reader;
        fn write(
            data: Vec<Self>,
            ...
            let mut groups: pco_pack::ahash::HashMap<(SmolStr,), Vec<usize>> = Default::default();
            for (i, rec) in data.iter().enumerate() {
                let key = (rec.device_id.clone(),);
                groups.entry(key).or_default().push(i);
            }
    ...
    impl pco_pack::PcoFilter for SmolStrIndex {
        fn filter_bulk(
            reader: &mut Self::Reader,
            field: usize,
            filter: &pco_pack::Filter,
        ...
        fn resolve_filter(
            ...
            match root {
                "" => {
                    ...
                    let sub_path = remainder.unwrap_or("");
                    let mut filter = <SmolStr as pco_pack::PcoFilter>::resolve_filter(
                        sub_path,
                        json,
                    )?;
                    if sub_path.is_empty() {
    ...
    impl pco_pack::PcoPack for SmolStrIndex {
        type Reader = Reader;
        type Chunk = Chunk;
        type Filter = Filter;
        fn write(data: Vec<Self>) -> pco_pack::anyhow::Result<Vec<Self::Chunk>> {
            ...
            let mut result: Vec<Chunk> = Vec::new();
            for (key, indices) in groups {
                for group_indices in indices.chunks(SmolStrIndex::CHUNK_SIZE) {
                    let mut col_temperature: Vec<i64> = Vec::new();
                    for &idx in group_indices {
                        let rec = &data[idx];
                        col_temperature.push(rec.temperature.clone());
    ...
    #[inline]
    fn expand_row(g: &mut Reader, row: usize) -> pco_pack::anyhow::Result<SmolStrIndex> {
        Ok(SmolStrIndex {
            device_id: g.device_id.clone(),
            temperature: g.temperature.pop_inner(row)?,
        })
    ...
    impl Chunk {
        ...
        /// Decompresses all payload columns and reconstructs the original rows.
        pub fn expand(self) -> pco_pack::anyhow::Result<Vec<SmolStrIndex>> {
            let mut reader = Reader {
                device_id: self.device_id.clone(),
                temperature: <pco_pack::LazyReader<
                    i64,
        ...
        pub fn filter(
            ...
            query: &pco_pack::serde_json::Value,
            fields: &[&str],
        ) -> pco_pack::anyhow::Result<Vec<SmolStrIndex>> {
            let fields = <SmolStrIndex as pco_pack::PcoPack>::resolve_fields(
                query,
                fields,
            )?;
            let execution_plan = <SmolStrIndex as pco_pack::PcoFilter>::resolve_query(
                query,
            )?;
            let mut reader = Reader {
                device_id: self.device_id.clone(),
            ...
            plan.sort_by_key(|&i| sizes[execution_plan[i].path[0]]);
            for i in plan {
                <SmolStrIndex as pco_pack::PcoFilter>::filter_step(
                    &mut reader,
                    &execution_plan[i].path,
                    &execution_plan[i].filter,
                    &mut matches,
