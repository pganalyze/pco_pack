...
const _: () = {
    ...
    /// Intermediate compressed form for [TimeSeriesCompact] with metadata fields
    ...
    pub struct Reader {
        pub database_id: i64,
        pub collected_at: pco_pack::LazyReader<Timeline<11_000_000>>,
    ...
    pub struct Writer {
        ...
        pub collected_at: Vec<pco_pack::Timeline<11_000_000>>,
        pub value: Vec<f64>,
    }
    ...
    impl pco_pack::PcoSerde for TimeSeriesCompact {
        type Writer = Writer;
        type Reader = Reader;
        fn write(
            data: Vec<Self>,
            ...
            for (key, mut indices) in groups {
                ...
                for &idx in indices.iter() {
                    if let Some(last) = group_rows.last_mut() {
                        let same = last.value == data[idx].value;
                        if same {
                            last.collected_at
                                .extend_ranges(data[idx].collected_at.ranges());
                            continue;
                        }
                    }
                    let mut ts_timeline = pco_pack::Timeline::<11_000_000>::new();
                    ts_timeline.extend_ranges(data[idx].collected_at.ranges());
                    group_rows
                        .push(Self {
                            collected_at: ts_timeline,
                            database_id: data[idx].database_id.clone(),
                            value: data[idx].value.clone(),
                        });
                }
                let mut col_collected_at: Vec<pco_pack::Timeline<11_000_000>> = Vec::new();
                ...
                let (g_start, g_end) = if !group_rows.is_empty() {
                    ...
                    for row in group_rows.iter() {
                        if row.collected_at.is_empty() {
                            return Err(
                                pco_pack::anyhow::anyhow!(
                                    "empty Timeline in timestamp field `{}` is not allowed",
                                    stringify!(collected_at)
                                ),
                            );
                        }
                        for &(s, e) in row.collected_at.ranges() {
                            if s < g_start {
                                g_start = s;
                            }
                            if e > g_end {
    ...
    impl pco_pack::PcoFilter for TimeSeriesCompact {
        fn filter_bulk(
            reader: &mut Self::Reader,
            field: usize,
    ...
    impl pco_pack::PcoPack for TimeSeriesCompact {
        type Reader = Reader;
        type Chunk = Chunk;
        type Filter = Filter;
        fn write(data: Vec<Self>) -> pco_pack::anyhow::Result<Vec<Self::Chunk>> {
            ...
            for (key, mut indices) in groups {
                ...
                for group_rows in group_rows.chunks(TimeSeriesCompact::CHUNK_SIZE) {
                    let mut col_collected_at: Vec<pco_pack::Timeline<11_000_000>> = Vec::new();
                    let mut col_value: Vec<f64> = Vec::new();
                    for rec in group_rows {
                        col_collected_at.push(rec.collected_at.clone());
    ...
    #[inline]
    fn timestamp_to_i64(v: &Timeline<11_000_000>) -> i64 {
        v.start().unwrap_or(0)
    }
    #[inline]
    ...
    fn expand_row(
        g: &mut Reader,
        row: usize,
    ) -> pco_pack::anyhow::Result<TimeSeriesCompact> {
        Ok(TimeSeriesCompact {
    ...
    impl Chunk {
        ...
        pub fn expand(self) -> pco_pack::anyhow::Result<Vec<TimeSeriesCompact>> {
            ...
            let loaded_fields: &[&str] = &["collected_at", "value"];
        ...
        pub fn filter(
            ...
            query: &pco_pack::serde_json::Value,
            fields: &[&str],
        ) -> pco_pack::anyhow::Result<Vec<TimeSeriesCompact>> {
            let fields = <TimeSeriesCompact as pco_pack::PcoPack>::resolve_fields(
                query,
                fields,
            )?;
            let execution_plan = <TimeSeriesCompact as pco_pack::PcoFilter>::resolve_query(
                query,
            )?;
            let mut reader = Reader {
                database_id: self.database_id.clone(),
            ...
            let sizes = [0, self.collected_at.len(), self.value.len()];
            plan.sort_by_key(|&i| sizes[execution_plan[i].path[0]]);
            for i in plan {
                <TimeSeriesCompact as pco_pack::PcoFilter>::filter_step(
                    &mut reader,
